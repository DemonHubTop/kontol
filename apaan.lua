...
local function Distance(pos)
    ...
    return round((head.Position - pos).Magnitude / 3)
end

local ESP_ID = tostring(math.random(100000,999999))

local function IslandESPHandler()
    for _,v in pairs(Workspace._WorldOrigin.Locations:GetChildren()) do
        pcall(function()
            local allow =
                (IslandESP and v.Name ~= "Sea")
                or (MirageIslandESP and v.Name == "Mirage Island")
                or (PrehistoricIslandESP and v.Name == "PrehistoricIsland")
            ...
            if plr ~= LP and plr.Character and plr.Character:FindFirstChild("Head") then
                ...
                if ESPPlayer then
                    if not head:FindFirstChild(tag) then
                        ...
                        head[tag].TextLabel.Text =
                            plr.Name.." | "..Distance(head.Position).." Distance\nHealth : "
...
local function FolderESP(folder, toggle, name)
        ...
        if v:FindFirstChild("HumanoidRootPart") then
            if toggle then
                if not v:FindFirstChild(name) then
                    ...
                    txt.TextColor3 = Color3.fromRGB(7,236,240)
                end
                v[name].TextLabel.Text =
                    v.Name.." - "..math.floor(
                        (LP.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude
                    ).." Distance"
            else
...
local function FruitAndFlowerESP()
    for _,v in pairs(Workspace:GetChildren()) do
        if DevilFruitESP and v:FindFirstChild("Handle") and v.Name:find("Fruit") then
            if not v.Handle:FindFirstChild("NameEsp"..ESP_ID) then
                local bill = Instance.new("BillboardGui", v.Handle)
                bill.Name = "NameEsp"..ESP_ID
                bill.Size = UDim2.new(1,200,1,30)
                bill.ExtentsOffset = Vector3.new(0,1,0)
                bill.AlwaysOnTop = true
                ...
                v.Handle["NameEsp"..ESP_ID].TextLabel.Text =
                    v.Name.." \n"..Distance(v.Handle.Position).." Distance"
            end
        end
    end
end

local function ChestESPHandler()
    for _,chest in pairs(CollectionService:GetTagged("_ChestTagged")) do
        if ChestESP and not chest:GetAttribute("IsDisabled") then
            if not chest:FindFirstChild("ChestEsp") then
                ...
                chest.ChestEsp.TextLabel.Text =
                    "Chest\n"..Distance(chest:GetPivot().Position).." M"
            end
        else
...
local function KitsuneIslandESPHandler()
    for _,v in pairs(workspace._WorldOrigin.Locations:GetChildren()) do
        pcall(function()
...
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            IslandESPHandler()
            KitsuneIslandESPHandler() -- ✅ KITSUNE ISLAND
            PlayerESPHandler()
            FolderESP(workspace.Enemies, MobESP, "MobEsp")
            FolderESP(workspace.NPCs, NpcESP, "NpcEsp")
            FolderESP(workspace.SeaBeasts, SeaESP, "SeaEsp")
            AfdESPHandler()
            FruitAndFlowerESP()
            ChestESPHandler()
        end)
    ...
    for _,v in pairs(LP.Backpack:GetChildren()) do
        if v:IsA("Tool") then
            ...
            elseif wtype == "Sword" and v.ToolTip == "Sword" then
                return v.Name
            elseif wtype == "Gun" and v.ToolTip == "Gun" then
                return v.Name
            elseif wtype == "Blox Fruit" and v.ToolTip == "Blox Fruit" then
                return v.Name
            end
        end
    end
...
task.spawn(function()
    while task.wait(0.5) do
        pcall(function()
            local weapon = GetWeaponByType(_G.SelectWeaponType)
            if weapon then
                _G.SelectWeaponName = weapon
            end
        end)
    end
end)
...
-- contoh equip / aimbot
if _G.SelectWeaponName then
    local tool =
        LP.Character:FindFirstChild(_G.SelectWeaponName)
        or LP.Backpack:FindFirstChild(_G.SelectWeaponName)
...
-- Elements inside Setting
_G.AutoFarm = false

local Tab = Main:Toggle({
    Title = "Auto Farm",
    Desc = "",
    Value = false,
    Callback = function(value)
        ...
        if _G.AutoNear then
            pcall(function()
                for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
                    ...
                    and game.Players.LocalPlayer.Character then
...
Main:Dropdown({
    ...
    Value = "Farm Level Mastery No Quest",
    Callback = function(v)
        _G.selectFruitFarm = v
        _G.UseSkill = false
        bringmob = false
    end
})
...
-- SEPARATOR
AutoFarm:Divider("Auto Chest Farm")

-- TOGGLE (WINDUI)
local Tab = Fram-other:Toggle({
    Title = "Auto Farm Chest",
    Desc = "Auto teleport to nearest chest",
    Value = false,
    Callback = function(v)
        _G.FarmChest = v
        StopTween(v)
            ...
            pcall(function()
                ...
                for _, Chest in ipairs(Chests) do
                    if not Chest:GetAttribute("IsDisabled") then
                        local mag = (Chest:GetPivot().Position - Position).Magnitude
...
-- SEPARATOR
AutoFarm:Divider("Auto Farm Bone")
...
-- SEPARATOR
AutoFarm:Divider("Hallow Scythe")
    ...
    while wait() do
        if _G.Hallow then
            pcall(function()
                ...
                elseif game.Players.LocalPlayer.Backpack:FindFirstChild("Hallow Essence")
                or game.Players.LocalPlayer.Character:FindFirstChild("Hallow Essence") then
...
Main:Separator("Tyrant of the Skies Farm")
...
local TyrantoftheSkies = CFrame.new(
    -16194.0048828125,
    155.21844482421875,
    1420.719970703125
)
...
-- =========================
-- KATAKURI / CAKE PRINCE
-- =========================

Main:Separator("Katakuri Farm")

-- STATUS LABEL
local Tab = Fram-other:Label({
    Title = "Check Cake Prince",
    Desc = "Checking spawn progress..."
})
    ...
    while task.wait(1) do
        pcall(function()
            ...
            if type(result) == "string" then
                ...
                if len == 88 then
                    CakePrinceStatus:Set(
                        "Killed : "..string.sub(result,39,41).." / 500"
                    )
                elseif len == 87 then
                    CakePrinceStatus:Set(
                        "Killed : "..string.sub(result,39,40).." / 500"
                    )
                elseif len == 86 then
                    CakePrinceStatus:Set(
                        "Killed : "..string.sub(result,39,39).." / 500"
                    )
                else
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        ...
                        and v.Humanoid.Health > 0 then
                            ...
                            repeat
                                ...
                                if workspace["_WorldOrigin"]:FindFirstChild("Ring")
                                or workspace["_WorldOrigin"]:FindFirstChild("Fist")
...
Main:Separator("Katakuri V2")

local Tab = Fram-other:Toggle({
    Title = "Auto Dough King",
    Desc = "But you have Sweet Chalice",
    Value = false,
    Callback = function(value)
    ...
    while task.wait() do
        if _G.Fullykatakuri then
            pcall(function()
                ...
                -- GOD CHALICE
                if bp:FindFirstChild("God's Chalice") or char:FindFirstChild("God's Chalice") then
                    if string.find(game.ReplicatedStorage.Remotes.CommF_:InvokeServer("SweetChaliceNpc"),"Where") then
                        game.ReplicatedStorage.Remotes.CommF_:InvokeServer("SweetChaliceNpc")
                    end

                -- SWEET CHALICE
                elseif bp:FindFirstChild("Sweet Chalice") or char:FindFirstChild("Sweet Chalice") then
                    if string.find(game.ReplicatedStorage.Remotes.CommF_:InvokeServer("CakePrinceSpawner"),
                        ...
                        for _,v in pairs(workspace.Enemies:GetChildren()) do
                            ...
                            and v.Humanoid.Health > 0 then
                                ...
                                repeat task.wait()
                                    ...
                                    game.VirtualUser:Button1Down(Vector2.new(1280,672))
                                until not _G.Fullykatakuri
                ...
                -- DOUGH KING
                elseif workspace.Enemies:FindFirstChild("Dough King")
                or game.ReplicatedStorage:FindFirstChild("Dough King") then
                ...
                -- RED KEY
                elseif bp:FindFirstChild("Red Key") or char:FindFirstChild("Red Key") then
                ...
                -- ELITE HUNTER
                    ...
                    if plr.PlayerGui.Main.Quest.Visible then
                        for _,v in pairs(workspace.Enemies:GetChildren()) do
                            if table.find({"Diablo","Deandre","Urban"},v.Name)
                            and v:FindFirstChild("Humanoid")
                            and v.Humanoid.Health > 0 then
...
Main:Separator("Auto Boss Farm")

_G.SelectBoss = _G.SelectBoss or ""
...
if World1 then
    ...
    BossList = {
        ...
        "Magma Admiral","Fishman Lord","Wysper",
...
elseif World3 then
    _G.SelectBoss = "Stone"
    BossList = {
        "Stone","Island Empress","Rocket Admiral",
        "Captain Elephant","Beautiful Pirate",
        "rip_indra True Form","Longma",
...
-- PARAGRAPH STATUS (GANTI LABEL)
local BossStatus = Main:Paragraph({
    Title = "Boss Status",
    Content = "Checking boss spawn..."
})
    ...
    while task.wait() do
        if _G.AutoBoss then
            pcall(function()
                ...
                if Enemies:FindFirstChild(_G.SelectBoss) then
                    for _,v in pairs(Enemies:GetChildren()) do
                        if v.Name == _G.SelectBoss
                        and v:FindFirstChild("Humanoid")
                        and v:FindFirstChild("HumanoidRootPart")
                        and v.Humanoid.Health > 0 then
                    ...
                    if Rep:FindFirstChild(_G.SelectBoss) then
                        topos(
                            Rep[_G.SelectBoss].HumanoidRootPart.CFrame
                            * CFrame.new(5,10,2)
                        )
                    end
                end
